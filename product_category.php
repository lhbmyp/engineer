...
/* 获取教师列表 */
$sql = "SELECT id, cat_id, product_name, price, content, product_image, add_time, description,sex,birth,title,subject,officephone,mail,office FROM " . $dou->table('product') . " WHERE cat_id IN (" .
         $cat_id . $dou->dou_child_id($dou->fetch_array_all('product_category'), $cat_id) . ") ORDER BY keywords" . $limit;
$query = $dou->query($sql);
...
$smarty->display('product_category.html');
?>